        ...
        try (BufferedReader br = new BufferedReader(new FileReader(ruta))) {
            ...
            while ((linea = br.readLine()) != null) {
                ...
                if (tipo.equals("LIBRO")) {
                    ...
                    // Si el archivo incluye un estado opcional en la columna 6, aplicarlo
                    if (p.length > 5) {
                        String estado = p[5].trim().toUpperCase();
                        if ("PRESTADO".equals(estado)) {
                            Material m = buscarMaterialPorId(id);
                            if (m != null && m.getEstado() == EstadoMaterial.DISPONIBLE) {
                                m.prestar();
                            }
                        }
                    }
